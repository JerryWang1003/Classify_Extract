...
def compute_geom_features(vertices: torch.Tensor, faces: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    angle_mean = np.mean(np.abs(np.dot(normals, normals.mean(axis=0))))
    if angle_mean < 0.5:  # 偏向 ±方向
        is_ring_like = 1
    else:   
        is_ring_like = 0
...
# 5) 主程式：訓練 + 推論 + 輸出 CSV（含特徵值）
# =========================
def train_and_eval(
    labels_csv="labels.csv",
    mesh_dir="obj_files/",
    out_csv="prediction_geom_with_feats.csv",
    num_classes=3,
    batch_size=32,
    num_epochs=40,
    lr=4e-4,
    ...
    for i, fname in enumerate(names):
        ...
        # 把特徵展開為可列印格式
        feat_values = X[i].detach().cpu().numpy().tolist()
        feat_values = [round(float(f), 4) for f in feat_values]  # 保留6位小數

        rows.append([fname, pred, int(y[i].item())] + plist + feat_values)

    # 標題列（前幾項為分類資訊，後面為幾何特徵名稱）
    feature_names = [
        "var_x", "var_y", "var_z",
        "linearity", "planarity", "anisotropy",
        "ex", "ey", "ez",
        "bbox_maxmin", "bbox_midmin", "axis_bending",
        "num_boundaries", "genus", "is_hollow"
    ]

    header = ["filename", "predicted_class", "label", "class_0_prob", "class_1_prob", "class_2_prob"] + feature_names
    ...
    print(f"已輸出 {len(rows)} 筆結果至 {out_csv}")

if __name__ == "__main__":
    train_and_eval(
        labels_csv="labels.csv",
        mesh_dir="obj_files/",
        out_csv="Feature_hole_1016.csv",
        num_classes=3,
        batch_size=32,
        num_epochs=40,
        lr=4e-4
